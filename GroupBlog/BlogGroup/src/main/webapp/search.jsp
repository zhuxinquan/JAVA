        ...
        <section>
            <div id="bbox">
                <%
                    ...
                    if(str != null && str.trim().length() != 0){
                        String sql = "select T_blog.Id, uid, Title, PubDate, ArticleDetail, Summary, Name from T_blog, T_user where T_user.Id = T_blog.uid and Title like '%" + str + "%' order by PubDate desc limit " + String.valueOf((Integer.parseInt(pageNum) - 1) * 10) + ", 10";
//                        System.out.println(sql);
        ...
        <div id="pagenum" class="pagenum">
            <%
                if(str != null && str.trim().length() != 0){
                    String sql = "select count(Id) num from T_blog where Title like '%" + str + "%'";
//                    System.out.println(sql);
                    ps = conn.prepareStatement(sql);
                    rs = ps.executeQuery();
                }
                ...
                if(list != 1){
            %>
            <a href="search.jsp?search=<%= str %>&page=1">首页</a>
            <a href="search.jsp?search=<%= str %>&page=<%= list - 1 %>">上一页</a>
            <%
                }
                if(list >= 6){
            %>
            <a href="search.jsp?search=<%= str %>&page=<%= (list / 5 - 1) * 5 + 1 %>">[…]</a>
            <%
                }
            ...
            %>
            <a href="search.jsp?search=<%= str %>&page=<%= i %>"><%= i %></a>
            <%
                    }
                }
                if((list - 1) / 5 * 5 + 1 != (count - 1) / 5 * 5 + 1){
            %>
            <a href="search.jsp?search=<%= str %>&page=<%= (list + 4) / 5 * 5 + 1 %>">[…]</a>
            <%
                }
                if(list != count && count != 0){
            %>
            <a href="search.jsp?search=<%= str %>&page=<%= list + 1 %>">下一页</a>
            <a href="search.jsp?search=<%= str %>&page=<%= count %>">尾页</a>
            <%
                }
            %>
        </div>
